...
export async function getContacts(userId: string): Promise<Contact[]> {
    return databases
        .listDocuments(DATABASE_ID, COLLECTION.contacts, [Query.equal("user_id", userId)])
        .then((result: { documents: any[]; }) => {
...
export async function getMessages(groupId: string): Promise<MessageModel[]> {
    return databases.listDocuments(DATABASE_ID, COLLECTION.messages, [
